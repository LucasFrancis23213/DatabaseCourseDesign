...
namespace DatabaseProject.BusinessLogicLayer.CommunityFeatureBLL
{
    public class CommunityFeatureBusiness<T> : ICommunityFeatureBusiness<T>
    {
        ...
        // 自定义from子句和where子句和select子句的方法
        public List<Dictionary<string,object>> QueryTableWithSelectBusiness(string selectClause,string fromClause,string whereClause, OracleParameter[] parameters)
        {
            try
            {
                // 调用 QueryWithCustomFromAndWhere 方法进行查询
                Tuple<bool, string> success = mergedDAL.QueryWithCustomSelect(selectClause,fromClause, whereClause, parameters);

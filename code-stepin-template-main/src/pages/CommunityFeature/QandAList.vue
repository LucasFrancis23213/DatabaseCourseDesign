<template>
  <a-card title="Questions List" class="conversations rounded-xl shadow-lg" :bordered="false">
    <div v-for="(question,qindex) in questionsList" :key="question.id"><!--用于测试样式：testdata_QL（定义请参见QandAtype.ts）;实际需用：questionsList"-->
...
import { item_id,current_user,Question,Answer,formatTime } from './QandAtype';

//import { testdata_QL } from './QandAtype';  //用于测试样式的数据，可注释，测试样式时，取消注释，并在template中替换questionsList
...
const fetchAnswers = async (question:Question) => {
  try {
    const response = await axios.post('/api/questions/{question_id}/answers', { question_id:question.id });
    if (response.data.status === 'success') {
...
const deleteAnswer = async (question: Question,answer_id: number,aindex: number) => {
  try {
    const response = await axios.post(`/api/questions_retract/{answer_id}`, { answer_id:answer_id, current_user_id:current_user.id, time: new Date() });
    if (response.data.status === 'success') {
...
const deleteQuestion = async (question_id: number) => {
  try {
    const response = await axios.post(`/api/answers_retract/{answer_id}`, { question_id:question_id, current_user_id:current_user.id, time: new Date() });
    if (response.data.status === 'success') {
      fetchQuestions(); // 重新获取问题列表
    }
  } catch (error) {

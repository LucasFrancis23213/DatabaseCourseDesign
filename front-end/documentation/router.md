...
> 本项目的路由使用 `vue-router` 来实现

### 路由配置

路由配置文件在 `src/router/routes.ts`，完全遵循 vue-router 的 `RouteRecordRaw` 类型定义。
...
另外，在 `src/router/dynamicRoutes.ts` 中，我们还提供了一些 api，以快速集成异步路由：
...
自定义菜单，给 `src/App.vue#stepin-view` 配置 menuList 即可：

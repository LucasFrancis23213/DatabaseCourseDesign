...
namespace DatabaseProject.BusinessLogicLayer.CommunityFeatureBLL
{
    public class CommunityFeatureBusiness<T>:ICommunityFeatureBusiness<T>
    {
        ...
        public T PackageData(params object[] parameters)
        {
            ...
            {
                throw new Exception($"Number of parameters ({parameters.Length}) does not match number of properties in type T ({properties.Length}).");
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                {
                    throw new Exception($"Parameter type ({value.GetType()}) does not match property type ({property.PropertyType}) at index {i}.");
                }
        ...
        // 映射数据
        private T MapDictionaryToObject(Dictionary<string, object> record)
        {
            T instance = Activator.CreateInstance<T>();
        ...
        // 查询事务
        public List<T> QueryBusiness(Dictionary<string,object> condition)
        {
            try
            {
                Tuple<bool, string> success = mergedDAL.QueryTable(tableName, condition);
                ...
                {
                    throw new Exception($"Error querying table: {success.Item2}");
                }
            }
            catch(Exception ex)
            {

...
namespace DatabaseProject.ServiceLayer.ConmmunityFeature
{
    public class QuestionAnswers
    {
        ...
        public List<Questions> GetQuestionsByItemId(int itemId)
        {
            try
            {
                var result = QuestionBusiness.QueryBusiness(new Dictionary<string, object> { { "item_id", itemId } },"AND");
        ...
        public List<Answers> GetAnswersLimitedByQuestionId(int questionId,int number)
        {
            ...
            {
                // 构建带有位置参数的查询语句
                string whereClause = $"question_id = :questionId AND ROWNUM <= :number";
                // 使用参数化查询
                ...
                List<Answers> queryResult = AnswerBusiness.QueryTableWithWhereBusiness("Answers", whereClause, parameters);
        ...
        public List<Answers> GetAnswersByQuestionId(int questionId)
        {
            try
            {
                var result = AnswerBusiness.QueryBusiness(new Dictionary<string, object> { { "question_id", questionId } }, "AND");

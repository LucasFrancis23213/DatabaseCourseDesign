    ...
    public class UserMessagesController : ControllerBase
    {
        ...
        public IActionResult GetConversations([FromBody] Dictionary<string,JsonElement> requestData)
        {
            try
            {
                ...
                foreach (var conversation in conversations)
                {
                    ...
                    var userInfo=conversation.userInfo;
                    var userMessage = conversation.userLastMessage;
                    // 根据会话中的 user1_id 和 user2_id 确定对方用户的 ID
                    int otherUserId = current_user_id == userConversation.User1_ID ? userConversation.User2_ID : userConversation.User1_ID;
                    ...
                    var conversationObject = new
                    {
                        ...
                        avatar = "",
                        last_message = userMessage != null ? userMessage.Message_Content : "", // 如果有最后一条消息则显示消息内容，否则为空字符串
                        last_message_time = userMessage != null ? userMessage.Send_Time : (DateTime?)null, // 如果有最后一条消息则显示发送时间，否则为 null
        ...
        public async Task<IActionResult> SendMessage([FromBody] Dictionary<string, JsonElement> request)
        {
            try
            {
                ...
                {
                    // 向所有客户端发送消息
                    await _hubContext.Clients.All.SendAsync("ReceiveMessage", success);
        ...
        public async Task<IActionResult> RetractMessage(int message_id, [FromBody] Dictionary<string, JsonElement> request)
        {
            try
            {
                ...
                if (success!=null)
                {
                    ...
                    // 向所有客户端发送撤回消息
                    await _hubContext.Clients.All.SendAsync("RetractMessage",success);

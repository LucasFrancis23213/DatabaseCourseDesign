    ...
    public class UserMessages
    {
        ...
        public List<Conversation_User_Message> FindAllUserConversations(int userId)
        {
            try
            {
                ...
                // 定义 WHERE 子句，包括查询条件
                string whereClause = $"USER1_ID=:userId OR USER2_ID=:userId";
                ...
                foreach (var row in rowList)
                {
                    ...
                    // 确定另一个用户的ID
                    int otherUserId = (conversation.User1_ID == userId) ? conversation.User2_ID : conversation.User1_ID;
                ...
                // 将合并后的会话转换为列表返回
                return mergedConversations.Values.ToList();
            }
            catch (Exception ex)
            {
